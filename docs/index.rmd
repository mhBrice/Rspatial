...
author: Marie-Hélène Brice <br> PhD student in ecology at Université de Montréal <br> [Pierre Legendre lab](http://adn.biol.umontreal.ca/~numericalecology/) & [Marie-Josée Fortin lab](https://fortin.eeb.utoronto.ca/) <br> <a href="http://qcbs.ca/"> <img src="https://qcbs.ca/wp-content/uploads/2010/05/image0032.png" style="height:34px;margin-top:5px"> </a>, <a href="https://insileco.github.io/"> <img src="./img/inSilecoLogo.png" style="height:34px;margin-top:5px;"> </a>
output:
...
```{r setup, echo=FALSE}
knitr::opts_chunk$set(fig.align = "center", fig.height = 5, cache = TRUE)
```

```{r packages, message=F, eval = T, echo=F, cache = F}
...
<i class="fa fa-code" aria-hidden="true"></i> [R code for this map](https://insileco.github.io/tuto/rinspace/rinspace_custommaps/)
...
### Possible limitations
  ...
  - Watershed analysis [<i class="fa fa-external-link" aria-hidden="true"></i>](http://ibis.geog.ubc.ca/~rdmoore/Rcode.htm)
  - ...
...
include_graphics("img/vector-data.png")
```
...
include_graphics("img/raster-data.png")
```
...
> See [dplyr and tidyr cheatsheet](https://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf) for other useful functions.
...
transform coordinate and re-project them, we would use `st_transform()` [<i class="fa fa-arrow-circle-o-right" aria-hidden="true"></i>](#76).
...
<li>Create a simple thematic map of watercourse `TYPE`. You can change the default colors using the argument `pal`. [<i class="fa fa-arrow-circle-o-right" aria-hidden="true"></i>](#44)
</ol>

## Responses

```{r read_courseau, echo=T, warning=F, message=F}
...
> <i class="fa fa-external-link" aria-hidden="true"></i> [Observatoire du Grand Montréal](http://cmm.qc.ca/donnees-et-territoire/observatoire-grand-montreal/produits-cartographiques/donnees-georeferencees/)
...
2. Transform it to a `sf` object using `st_as_sf()` [<i class="fa fa-arrow-circle-o-right" aria-hidden="true"></i>](#28)

3. Subset the Quebec boundary in the column `NAME_1` [<i class="fa fa-arrow-circle-o-right" aria-hidden="true"></i>](#52)

4. Re-project your new object using the same projection that of the land use polygons (`st_crs(LU_val)` or with the EPSG code `32188`) [<i class="fa fa-arrow-circle-o-right" aria-hidden="true"></i>](#76)
...
```{r getdata2, echo=T, warning=F, fig.height=4.8, fig.width=9}
...
1. Re-project the `courseau` object using the land use projection and name it `courseau_proj` [<i class="fa fa-arrow-circle-o-right" aria-hidden="true"></i>](#76)

2. Create a 300m buffer around each watercourse and name it `courseau_buf`. Plot the resulting geometry with `courseau_proj` on top. [<i class="fa fa-arrow-circle-o-right" aria-hidden="true"></i>](#77)
...
> For more geometric operations such as `st_distance()`, `st_convex_hull()`, `st_intersection()` see [sf vignette #3](https://cran.r-project.org/web/packages/sf/vignettes/sf3.html)
...
```{r exo_buf3.1, echo = F, fig.height =4.5}
buf_union <- st_union(courseau_buf)
...
```{r exo_buf4.1, echo = F, fig.height =4.5}
# compute centroid
ruisso_centroid <- st_centroid(ruisso_buf)
par(mar=c(0,0,3,0))
plot(st_geometry(ruisso_buf), col=NA, border="black", main = "st_centroid")
...
> Classification and color codes of land use types [<i class="fa fa-external-link" aria-hidden="true"></i>](http://cmm.qc.ca/fileadmin/user_upload/geomatique/UtilisationDuSol/UtilSol_Legende.pdf)
...
> Reshape data with melt and cast [<i class="fa fa-external-link" aria-hidden="true"></i>](https://datascienceplus.com/data-manipulation-in-r-with-reshape2/)
...
```{r plot_crop2, echo=F, message = F, fig.align="center", fig.height=3.5, fig.width=10}
par(mar=c(3,3,1,1.5), mfrow=c(1,3))
plot(altCAN)
plot(alt_crop)
plot(alt_mask)
plot(st_geometry(ruisso_buf), add = T)
...
> See the package `velox` [<i class="fa fa-external-link" aria-hidden="true"></i>](https://hunzikp.github.io/velox/extract.html) for faster raster extraction
...
Change the color palette for the raster levels. There are 5 levels [<i class="fa fa-external-link" aria-hidden="true"></i>](http://cmm.qc.ca/fileadmin/user_upload/geomatique/IndiceCanopee/2015/CMM_indiceCanopee_2015_methodologie.pdf) and only 2 are pertinent:
...
<div align="center">
```{r mapview31, echo=F, cache=F}
myPal <- colorRampPalette(brewer.pal(9, "YlGnBu"))
mapview(ruisso_env, zcol = "resid", cex = "pH", legend = TRUE, col.regions = myPal, layer.name = "% of residential area")
```
</div>
