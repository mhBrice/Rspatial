---
title: "Spatial analyses and mapping in R"
...
```{r export, warnings = FALSE, message=FALSE, results="hide"}
...
intersecting, holes being inside polygons, less than 4 points, non-closing
...
courseau <- st_read(dsn = "data/courseau/", layer = "courseau")
```
...
The R package raster provides three main classes of raster object (more details [here](http://www.rspatial.org/spatial/rst/4-rasterdata.html#rasterlayer)):
...
The canopy index raster has values from 1 to 5, has `r nrow(canopee_mtl)` pixels
...
```{r plot_altcan, fig.height=4, fig.width=4.5}
plot(altCAN)
```
...
plot(st_geometry(ruisso_centroid), pch = 21, col="red", add = T)
```
